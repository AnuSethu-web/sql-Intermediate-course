...
WHERE
    ...
    AND
    (AdmittedDate >= ' 2024-02-01' AND AdmittedDate <'2024-03-01')
    AND
    Ward LIKE 'D%'
...
4. Add a calculated column called LengthOfStay that shows the number of days 
...
FROM
    PatientStay ps
ORDER BY
AdmittedDate DESC, PatientId DESC
/*
...
SELECT
    ps.Hospital
    ,COUNT(PatientId) AS NumberOfPatients ,SUM(Tariff) AS TotalTariff
FROM
    PatientStay ps
